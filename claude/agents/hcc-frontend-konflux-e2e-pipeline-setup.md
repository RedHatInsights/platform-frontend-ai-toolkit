...
**Step 6a: Verify Required Parameters from Shared Pipeline**
...
- `unit-tests-script` - Script to run unit tests
- `e2e-tests-script` - Script to run E2E tests
- `run-app-script` - Script to start the application
- `playwright-image` - Playwright Docker image (must match package.json version)
...
**Step 6b: Check for Existing Pipeline**
...
**Step 6c: Modify Existing Pipeline (if found)**
...
- `unit-tests-script` - Script to run unit tests (REQUIRED)
- `e2e-tests-script` - Script to run E2E tests (REQUIRED)
- `run-app-script` - Script to start the application (REQUIRED)
- `playwright-image` - Playwright Docker image matching package.json version (REQUIRED)
...
**Step 6d: Create New Pipeline (if none exists)**
...
**Step 9: Validate Pipeline Configuration**
...
- [ ] All REQUIRED parameters included (unit-tests-script, e2e-tests-script, run-app-script, playwright-image, frontend-proxy-routes-configmap, e2e-credentials-secret)
...
**Step 10: Submit Changes in Correct Order**

**CRITICAL: Follow this two-step process to avoid cryptic errors:**

**Step 10a: Submit and Merge konflux-release-data PR First**
...
**Step 10b: Submit Repository PR with Pipeline and Tests**
...
- [ ] Required pipeline parameters are configured:
  ...
  - [ ] `playwright-image` - Playwright Docker image matching package.json version (REQUIRED, e.g., `mcr.microsoft.com/playwright:v1.40.0-jammy`)
  - [ ] `frontend-proxy-routes-configmap` - ConfigMap name for proxy routes (REQUIRED, simplified routing to app port, all else proxies to stage)
  - [ ] `e2e-credentials-secret` - Secret name for test credentials (REQUIRED)
  - [ ] `test-app-name` - Application name for routing (optional)
  - [ ] `test-app-port` - Port for app assets (optional, default: 8000)
