...
2. **ALWAYS use Plumber to generate ConfigMaps** - never write them manually. Run Plumber with `uv run plumber` (not manual pip install)
3. **ALWAYS follow the two-step PR process** - Submit and merge the konflux-release-data MR with ConfigMaps and ExternalSecrets FIRST, then submit the repository PR with pipeline changes SECOND. Submitting the pipeline PR first will cause cryptic "resource not found" errors.
4. **NEVER skip verification steps** - each configuration change should be validated
5. **ALWAYS reference existing working examples** from learning-resources or frontend-starter-app
6. **NEVER assume the developer has access to internal resources** - ask first before referencing internal documentation
7. **ALWAYS validate required prerequisites** are in place before proceeding
8. **NEVER modify the shared pipeline definition** unless absolutely necessary - focus on repository-specific configuration
9. **ALWAYS check for existing pipeline files** - modify existing pull request pipelines in place rather than creating duplicates with different filenames. Konflux validates all `.tekton/*.yaml` files and will reject duplicate PipelineRun names regardless of filename.
10. **IMPORTANT: Chrome sidecar has been removed** - The E2E pipeline no longer uses a separate insights-chrome-dev sidecar. All non-app routes now proxy directly to the actual stage environment. This greatly simplifies the proxy ConfigMap.
11. **IMPORTANT: App sidecar ConfigMap no longer needed** - The shared pipeline has been updated so most repositories no longer require a separate ConfigMap for the test application sidecar. Only the frontend-developer-proxy ConfigMap is needed in most cases.
12. **IMPORTANT: Plumber now generates only one ConfigMap** - The test application configmap is no longer generated by Plumber. Only the proxy ConfigMap is produced.
...
**7a-i. If Repository Not Cloned Locally (Optional Agent Assistance)**
...
**Step 9: Submit Changes in Correct Order**

**CRITICAL: Follow this two-step process to avoid cryptic errors:**

**Step 9a: Submit and Merge konflux-release-data PR First**
...
**Step 9b: Submit Repository PR with Pipeline and Tests**
...
**VERSION COMPATIBILITY:** The `playwright-image` parameter in your pipeline MUST match the version of `@playwright/test` in your package.json:
- package.json has `"@playwright/test": "^1.40.0"` → use `playwright-image: "mcr.microsoft.com/playwright:v1.40.0-jammy"`
- package.json has `"@playwright/test": "^1.45.0"` → use `playwright-image: "mcr.microsoft.com/playwright:v1.45.0-jammy"`
...
Causes:
- The `playwright-image` parameter in the pipeline uses a different version than `@playwright/test` in package.json
...
2. Update the `playwright-image` parameter in your pipeline to match:
   ```yaml
   - name: playwright-image
...
**CRITICAL:** Always keep these versions synchronized. When you upgrade Playwright in package.json, also update the pipeline's playwright-image parameter.

**Common Issue 9: Chrome Sidecar Removal Migration**
...
- [ ] Required E2E pipeline parameters are configured:
  ...
  - [ ] `playwright-image` - Playwright Docker image matching package.json version (e.g., `mcr.microsoft.com/playwright:v1.40.0-jammy`)
